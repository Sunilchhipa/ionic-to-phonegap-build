...
exports.sendToPhonegapBuild = function sendToPhonegapBuild(phonegapAppId, phonegapAuthToken, isDebug) {
    ...
    var copyFiles = function () {
        ...
        copyfiles(['www/**', `${config.folder}/package`], '', copyConfig);

    }

    var copyConfig = function () {
        console.log("Copying configuration file...".gray);
        copyfiles(['config.xml', `${config.folder}/package/www`], '', copyAndroidFCMFile);
    }

    var copyAndroidFCMFile = function () {
        console.log("Copying Android FCM file...".gray);
        copyfiles(['google-services.json', `${config.folder}/package/www`], '', copyiOSFCMFile);
    }

    var copyiOSFCMFile = function () {
        console.log("Copying iOS FCM file...".gray);
        copyfiles(['GoogleService-Info.plist', `${config.folder}/package/www`], '', copyResources);
    }
	
    var copyResources = function () {
        console.log("Copying resources files for package...".gray);
        copyfiles(['resources/**', `${config.folder}/package/www`], '', zipFiles);
    }
    ...
    var finished = function (e, a) {
        if (e !== null) {
            ...
            del.sync([config.folder + '/' + config.archiveName]);
        }
